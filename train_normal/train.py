    ...
    def save_final_model():
        ...
        if accelerator.is_main_process:
            ...
            if args.inference_parameters is not None:
                model_name = args.inference_parameters["model_name"]
                ...
                sagemaker_session = sagemaker.Session(
                    boto3.session.Session(region_name="eu-central-1"))
                logging.info(sagemaker_session.boto_region_name)
                # make sure this is set and not None
                        ...
                        for i in tqdm(range(n_inferences), desc="Generating samples"):
                            ...
                            for image_file in images:
                                ...
                                s3_client.put_object(
                                    ...
                                    Key=f"{s3_results_storage_key}/{model_name}/{idx}-{get_random_hash()}",
                                    Body=buffer,
                ...
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                logging.info(
                    f"[*] Final model saved at {args.model_output_dir}")
