...
def parse_args(input_args=None):
    ...
    # Inference parameters
    parser.add_argument(
        "--inference_images_storage_dir", 
        default="",
        type=str,
        help=" Folder where the inference results will be stored."
    )
    parser.add_argument(
        "--depthmap_images_storage_dir", 
    ...
    inference_parameters = {
        "inference_images_storage_dir": args.inference_images_storage_dir,
        "depthmap_images_storage_dir": args.depthmap_images_storage_dir,
        "prompt": args.prompt,
    ...
    def save_final_model():
        ...
        if accelerator.is_main_process:
            ...
            if args.inference_parameters is not None:
                inference_images_storage_dir = args.inference_parameters["inference_images_storage_dir"]
                depthmap_images_storage_dir = args.inference_parameters["depthmap_images_storage_dir"]
                ...
                sample_images_names = os.listdir(depthmap_images_storage_dir)
                sample_images = []
                for filename in sample_images_names:
                    logging.info(filename)
                    if "JPG" in filename or "jpeg" in filename or "jpg" in filename:
                        print("Generating new images")
                        image = Image.open(os.path.join(depthmap_images_storage_dir, filename))
                    ...
                    with torch.autocast("cuda"), torch.inference_mode():
                        for i in tqdm(range(n_inferences), desc="Generating samples"):
                            ...
                            for image_file in images:
                                image_file.save(f"{inference_images_storage_dir}/{get_random_hash()}")
