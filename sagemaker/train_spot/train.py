...
def parse_args(input_args=None):
    ...
    #    type=dict,
    #    default=None,
    #    help="Parameters for inference with final model",
    #)
    ...
    # Inference parameters
    parser.add_argument(
        "--s3_results_storage_bucket", 
        default="sagemaker-user-io",
        type=str,
        help="Bucket where the inference results will be stored."
    )
    parser.add_argument(
        "--s3_results_storage_key", 
        default="",
        type=str,
        help=" Folder where the inference results will be stored."
    )
    parser.add_argument(
        "--s3_depthmap_images_storage_bucket", 
        default="sagemaker-constants",
        type=str,
        help="Bucket where the input (depth masks) to the inference pipeline are stored."
    )
    parser.add_argument(
        "--s3_depthmap_images_storage_key", 
        default="depthmask_images/man",
        type=str,
        help="Folder where the input (depth masks) to the inference pipeline are stored."
    )
    parser.add_argument(
        "--prompt", 
        default="",
        type=str,
        help="Positive text input."
    )
    parser.add_argument(
        "--negative_prompt", 
        default="",
        type=str,
        help="Negative text input."
    )
    parser.add_argument(
        "--guidance_scale", 
        default=7.5,
        type=float,
        help="How much the text influences the output."
    )
    parser.add_argument(
        "--infer_steps", 
        default=70,
        type=int,
        help="Denoising steps for one inference repetition."
    )
    parser.add_argument(
        "--n_inferences", 
        default=6,
        type=int,
        help="How many inference repetitions should be performed for a given depthmap input. Each inference repetition will produce num_images_per_prompt outputs."
    )
    parser.add_argument(
        "--num_images_per_prompt", 
        default=5,
        type=int,
        help="Number of images to produce per inference repetition."
    )
    ...
    inference_parameters = {
        "s3_results_storage_bucket": args.s3_results_storage_bucket,
        "s3_results_storage_key": args.s3_results_storage_key,
        "s3_depthmap_images_storage_bucket": args.s3_depthmap_images_storage_bucket,  
        "s3_depthmap_images_storage_key": args.s3_depthmap_images_storage_key,
        "prompt": args.prompt,
    ...
    args.inference_parameters = inference_parameters
    return args
...
def main(args):
    ...
    if len(os.listdir(args.model_checkpoint_dir)) != 0:
        folders = sorted([f for f in os.listdir(args.model_checkpoint_dir) if f != "0"], key=lambda x: int(x))
        checkpoint_epoch = int(folders[-1])
        logger.info(f"***** continue training from checkpoint {checkpoint_epoch} *****")
        args.pretrained_depth_model_path = os.path.join(args.model_checkpoint_dir, str(checkpoint_epoch))
    else:
    ...
    def save_checkpoint(step):
        ...
        if accelerator.is_main_process:
            ...
            """if args.save_sample_prompt is not None:
                pipeline = pipeline.to(accelerator.device) #y
                g_cuda = torch.Generator(device=accelerator.device).manual_seed(args.seed) #y
                pipeline.set_progress_bar_config(disable=True) #y
                sample_dir = os.path.join(save_dir, "samples") #y
                os.makedirs(sample_dir, exist_ok=True) #y
                # TODO: Use own image
                sample_img = None
                if args.save_sample_image is None:
                    url = "https://ca.slack-edge.com/TUTJQ5RNV-U02P1ET637V-7d91d73a4fd4-512"
                    sample_img = Image.open(requests.get(url, stream=True).raw)
                else:
                    sample_img = Image.open(args.save_sample_image)
            ...
            # MAYBE: The author added the following two lines:
    ...
    def save_final_model():
        ...
        if accelerator.is_main_process:
            ...
            if args.inference_parameters is not None:
                s3_results_storage_bucket = args.inference_parameters["s3_results_storage_bucket"]
                s3_results_storage_key = args.inference_parameters["s3_results_storage_key"]
                s3_depthmap_images_storage_bucket = args.inference_parameters["s3_depthmap_images_storage_bucket"]
                s3_depthmap_images_storage_key = args.inference_parameters["s3_depthmap_images_storage_key"]
                negative_prompt = args.inference_parameters["negative_prompt"]
                prompt = args.inference_parameters["prompt"]
                guidance_scale = args.inference_parameters["guidance_scale"]
                infer_steps = args.inference_parameters["infer_steps"]
                seed = args.inference_parameters["seed"]
                n_inferences = args.inference_parameters["n_inferences"]
                num_images_per_prompt = args.inference_parameters["num_images_per_prompt"]

                # When running on SageMaker, need execution role
                sagemaker_session = sagemaker.Session(boto3.session.Session(region_name="eu-central-1"))
                logging.info(sagemaker_session.boto_region_name)
                print(sagemaker_session.boto_region_name) # make sure this is set and not None
                ...
                logging.info(sagemaker_execution_role)
                s3 = boto3.resource('s3')
                s3_client = boto3.client('s3')
                # Download all sample images inside s3
                s3_sample_images_names = s3_client.list_objects(
                    Bucket=s3_depthmap_images_storage_bucket, 
                    Prefix=s3_depthmap_images_storage_key)['Contents']
                s3_sample_images = []
                for f in s3_sample_images_names:
                    logging.info(f['Key'])
                    if "JPG" in f['Key'].split("/")[1] or "jpeg" in f['Key'].split("/")[1]:
                        logging.info("Loop!")
                        image = image_from_s3(s3_depthmap_images_storage_bucket,f['Key'],s3)
                        s3_sample_images.append(image)
                
                pipeline = pipeline.to(accelerator.device)
                g_cuda = torch.Generator(device=accelerator.device).manual_seed(seed)
                pipeline.set_progress_bar_config(disable=True)
                for sample_img in s3_sample_images:
                    with torch.autocast("cuda"), torch.inference_mode():
                        for i in tqdm(range(n_inferences), desc="Generating samples"):
                            images = pipeline(
                                prompt=prompt,
                                image=sample_img,
                            ...
                            for image_file in images:
                                buffer = io.BytesIO()
                                image_file.save(buffer, "JPEG")
                                buffer.seek(0) # rewind pointer back to start
                                s3_client.put_object(
                                    Bucket=s3_results_storage_bucket,
                                    Key=f"{s3_results_storage_key}/{get_random_hash()}",
                                    Body=buffer,
                ...
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                logging.info(f"[*] Final model saved at {args.model_output_dir}")
